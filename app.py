...
# ========== ВРЕМЕННО: ОЧИСТКА СТАРЫХ ДАННЫХ (удалить или закомментировать после 1-го запуска) ==========
#DATA_FILE = "projects.xlsx"
#if os.path.exists(DATA_FILE):
   # os.remove(DATA_FILE)
    #st.warning("Файл projects.xlsx удалён. Теперь можно импортировать или создавать новые проекты.")
    # st.stop()  # при необходимости раскомментируйте, чтобы остановить выполнение после удаления
# =======================================================================================
...
if page == "📋 Проекты":
    ...
    with col2:
        ugt_filter = st.selectbox("УГТ", ["Все"] + sorted(df["ugt"].unique()) if not df.empty else ["Все"])
    with col3:
